  ...
  return (
    <div>           
      ...
      <section className={'py-8 w-full fixed z-50 backdrop-blur duration-200 bg-zinc-900/500 border-b border-zinc-600'}>
        ...
        <Link href="/portfolio" aria-label="Back to portfolio page"><ArrowLeftIcon className="relative z-20 w-8 text-zinc-400 hover:text-white" /></Link>
              <div className="flex gap-6">
                <Link href={item.github} target="_blank" aria-label="Link to Github">              
                  <svg width="25" height="25" viewBox="0 0 1920 1920" xmlns="http://www.w3.org/2000/svg" alt={item.github} className="fill-zinc-400 hover:fill-white">
                ...
                <div>
                  <Locales paramSlug={`/portfolio/${slug}`}/>              
                </div>
              </div>            
        </div>
      </section>
            ...
            <section className="py-28 flex flex-col gap-8 text-center animate-fadeIn">
              ...
              <p className="text-zinc-400 lg:w-1/3 m-auto">{pathname.includes("/en/") ? item.descriptionEN : item.description}</p>
          ...
          <div className="w-full py-20"> 
            <div className="w-4/5 m-auto flex flex-col gap-8 items-center">
              ...
              <Technologies sendData={item} usedTechnology={pathname.includes("/en/") ? "Technologies used" : "Použité technológie"}/>
            </div>
          </div>
          </div>
        );
